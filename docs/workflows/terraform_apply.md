...
| `iam_group` | `string` | ❌ | Name for the IAM group used in Vault to retrieve secrets. Default: canonical-webdesign. |
...
| `TERRAFORM_REPO_SSH_KEY` | ❌ | If you followed the `docs/ps7_setup.md` "Accessing private repositories" section you need to set the private SSH key in a secret with this name. |
...
```yaml
  apply:
    ...
    with:
      ...
      iam_group: canonical-webdesign-marketplace
      vault_model_name: k8s-stg-marketplace-rocks
    secrets:
